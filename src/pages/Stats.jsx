...
useEffect(() => {
  ...
  <button onClick={()=>setScanOpen(true)} style={{display:'flex',alignItems:'center',gap:8,padding:'10px 20px',borderRadius:12,border:'none',background:'#e8724a',color:'white',fontSize:14,fontWeight:700,cursor:'pointer'}}>
    📷 Scan Device
  </button>
</div>
...
{scanOpen && (
  <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',backdropFilter:'blur(6px)',zIndex:100,display:'flex',alignItems:'center',justifyContent:'center'}}>
    <div style={{background:'white',borderRadius:24,padding:36,width:360,textAlign:'center',boxShadow:'0 20px 60px rgba(0,0,0,0.25)'}}>
      <div style={{fontSize:13,fontWeight:700,color:'#e8724a',marginBottom:8,textTransform:'uppercase',letterSpacing:1}}>Add Device</div>
    ...
    <div style={{width:260,margin:'0 auto 20px',borderRadius:16,overflow:'hidden',border:'3px solid #e8724a'}}>
  <video
    ref={videoRef}
  ...
  />
      ...
      <button onClick={()=>setScanOpen(false)} style={{width:'100%',padding:13,borderRadius:12,border:'none',background:'#f0f0f0',color:'#666',fontSize:14,fontWeight:700,cursor:'pointer'}}>
        Cancel
      </button>
    </div>
  </div>
